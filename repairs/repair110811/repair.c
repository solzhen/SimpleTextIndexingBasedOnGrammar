...
RDS *createRDS(FILE *input)
{
  ...
  size_w = ftell(input); // get the current position
  rewind(input); // move to the beginning of the file
  seq = (SEQ*)malloc(sizeof(SEQ)*size_w); // allocate memory for the text

  printf("text size = %d(bytes)\n", size_w);

  i = 0;
  while ((c = getc(input)) != EOF) { // read the text
    seq[i].code = c;
    seq[i].next = DUMMY_POS;
  ...
  h_num = INIT_HASH_NUM;
  h_first = (PAIR**)malloc(sizeof(PAIR*)*primes[h_num]); // allocate memory for the hash table
  for (i = 0; i < primes[h_num]; i++) {
    h_first[i] = NULL;
  }

  p_max = (uint)ceil(sqrt((double)size_w)); // max frequency
  p_que = (PAIR**)malloc(sizeof(PAIR*)*p_max); // allocate memory for the priority queue
  for (i = 0; i < p_max; i++) {
    p_que[i] = NULL;
  }
